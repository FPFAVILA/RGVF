...
export const GameDashboard: React.FC<GameDashboardProps> = ({ user }) => {
  ...
  const handleAddBalance = (amount: number) => {
    // Se for deposito de verificacao KYC (qualquer valor)
    if (kycDepositAmount !== null && amount > 0) {
      // Creditar o valor
      addBalance(amount);

      // Marcar KYC como verificado
      const updatedKYC = {
        ...gameState.kycStatus,
    ...
    <div className={`min-h-screen bg-primary safe-area-top safe-area-bottom transition-opacity duration-300 ${isInitialLoad ? 'opacity-0' : 'opacity-100'}`}>
      ...
      <AddBalanceModal
        ...
        message={kycDepositAmount ? (
          'Faca um deposito de qualquer valor para confirmar sua identidade. O valor sera creditado no seu saldo e os saques serao liberados automaticamente.'
        ) : !canPlay ? (
